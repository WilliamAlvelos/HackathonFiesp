...
class ChatGroup:
    """
    ChatGroup class as defined by Telegram API at https://core.telegram.org/bots/api#groupchat
    ...
    def __init__(self, groupData):
        """
        (dict) -> constructor
        ChatGroup class constructor. 'groupData' must be a valid JSON representation of group information
        """
    ...
    def __str__(self):
        """
        () -> str
        Human readable representation for ChatGroup object
        """
        string = "Group: " + self.title
        return string
...
class Message:
    ...
    classes: PhotoSize, Audio, Document, Sticker, Video, Contact, Location, Update, InputFile, UserProfilePhotos.
    ...
    def __init__(self, messageData):
        ...
        if "title" in messageData["chat"]:
            self.chat = ChatGroup(messageData["chat"])
        else:
            self.chat = User(messageData["chat"])
